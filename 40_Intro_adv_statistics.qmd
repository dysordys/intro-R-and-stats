...
:::{.callout-important}
If multiple explanatory variables are assumed to affect the response variable, the linear model is extended with several $\beta_j$, one for each explanatory variable $X_j$. More on that in @sec-adv-reg.
:::
