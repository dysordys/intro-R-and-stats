...
3.  Load and display the limerick that you saved in Exercise 5 of @sec-intro-exercises. Follow its instructions and verify that the result is indeed "nine squared and not a bit more". (Hint: one dozen is 12, one gross is a dozen dozens, or 144, and one score is 20.)
...
5.  Applying the same rule backwards: what are the frequencies of the alleles **A** and **a** if 49% of the population has brown eyes, 21% have green eyes, and 9% have blue eyes? 
...
7.  As populations are selected for some trait, it is common for the available genetic variance to eventually diminish (since certain genotypes are being discarded from the population). This leads to a reduction in heritability over time. Repeat the above calculations, but assuming that the heritability $h^2$ is reduced by 10% after every round of selective breeding.
8.  Create a vector called `z`, with entries 1.2, 5, 3, 13.7, 6.66, and 4.2 (in that order). Then, by applying functions to this vector, obtain:
    - Its smallest entry.
    - Its largest entry.
...
9.  Define a vector of strings, called `s`, with the three entries `"the fat cat"`, `"sat on"`, and `"the mat"`.
