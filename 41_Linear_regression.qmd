...
```{r}
#| fig-cap: Distribution of a sample from other distributions
#| fig-height: 3
#| fig-width: 5 
set.seed(1234)
...
These plots suggest that the model is missing an explanatory variable or needs to be transformed in some way to meet the assumption.

If the QQ plot shows clear patterns that deviate from the line --- for example if the points curve around the line --- it means the model does not meet the assumption of a linear relationship.
...
### Creating a custom function (`diagnosticPlots`)
...
7. Create diagnostic plots to check whether the 
