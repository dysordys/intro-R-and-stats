...
As we saw in @fig-dist-mean, the measure of a center by itself does not provide the full picture of a variable. Species 1 and 2 had about the same mean value but the values of the observations are spread very different. When summarizing a variable it is also important to describe the spread of the values, the amount of variation they have, as it gives a sense of how two variables with the same mean might differ.
...
To get an average (squared) distance, we then divide by the number of observations or at least something that depends on the number of observations.^[We will return to why we do not use $n$ directly in a later chapter.]
