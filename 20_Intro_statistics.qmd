...
A qualitative variable can be visualized using a bar plot---or, if a bar plot is not appropriate (@sec-CI), then by using points. If the variable follows the ordinal scale, the categories on the x-axis should follow the same order. If the variable follows the nominal scale we do not have to follow any specific order of the categories, but it is usually nice to order them either alphabetically or in descending order based on the (relative) frequency of the category.
...
We can calculate the position of the median as (*n* + 1) / 2, where *n* is the number of data points. This results in either a whole number (if *n* is odd), in which case the value at that position is the median. Or it could result in a half number (if *n* is even), in which case the convention is to put the median halfway between the two positions obtained by rounding the position's number up and down, respectively.
...
pop |>
  ...
  slice(c(floor(position), ceiling(position))) |> # Get the middle two values
  summarize(median = mean(species1)) # Obtain the median
```
...
pop |> 
  ...
  # Adds mean values to the dataset prior to visualization
  group_by(Species) |> 
  mutate(
    mean = mean(Density)
  ) |> 
