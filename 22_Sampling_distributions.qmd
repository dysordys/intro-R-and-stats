...
We can see that the sample mean (light blue line) overestimates the population mean (dark red line) but if we were to draw another sample of the same size the sample mean would be a different value.
...
$$ {#eq-samp-dist-mean}
where the mean of the sample mean ($\mu_{\bar{X}}$) is the same as the mean of X, and the standard deviation of the sample mean ($\sigma_{\bar{X}}$) is the standard deviation of X divided by the sample size n.
