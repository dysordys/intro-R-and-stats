...
Similar to the output from the one-sample t-test we can compare the p-value to the level of significance in order to determine if the null hypothesis can be rejected or not. In this example the p-value of 0.03468 is lower than the significance level of 0.05 which means we reject $H_0$. We then conclude that with 5% significance we have found evidence in support of the insular dwarfism theory.^[In this case we know that it is not true given we created the sample ourselves, so we have now encountered a type I error in practice.]
...
We find that the p-value, given our data, is 0.02794. What this number means is that we have a roughly one-in-thirtysix chance that the observed difference is nothing but a fluke. Comparing this to the level of significance we conclude that $H_0$ can be rejected.
...
In summary, the tests yielded evidence in favor of rejecting the null hypothesis (in support of the alternative). Therefore, we can conclude that with 5% significance our fictive birds have smaller sizes on the smaller island. In this particular case, since we ourselves have created the original data using the null hypothesis, we have the privilege of knowing that this is false. When working with real data, such knowledge is generally not available.
